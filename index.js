...
const LAST_GOOD_DATA_FILE = path.join(__dirname, "data", "family.last_good.json");
...
writeJsonIfMissing(LAST_GOOD_DATA_FILE, { nodes: {} });
...
function mirrorLastGoodFamily(data) {
  if (getNodeCount(data) === 0) return false;
  writeJson(LAST_GOOD_DATA_FILE, data);
  return true;
}
...
function createFamilyBackup(reason = "change") {
  ...
  const nodeCount = getNodeCount(currentData);
  ...
  mirrorLastGoodFamily(currentData);
...
function restoreLatestBackupIfFamilyIsEmpty() {
  ...
  const currentNodeCount = getNodeCount(currentData);

  if (currentNodeCount > 0) return false;

  const backups = getBackupFiles();
  if (backups.length === 0) {
    const lastGoodData = readJson(LAST_GOOD_DATA_FILE, null);
    if (getNodeCount(lastGoodData) > 0) {
      cleanupFamily(lastGoodData);
      writeJson(DATA_FILE, lastGoodData);
      console.warn("🛡️ Arbre restauré automatiquement depuis family.last_good.json.");
      return true;
    }
    return false;
  }
  ...
  for (const backup of backups) {
    ...
    const backupNodeCount = getNodeCount(backupData);
...
function saveFamily(data, options = {}) {
  cleanupFamily(data);

  const nextNodeCount = getNodeCount(data);
  const currentData = readJson(DATA_FILE, { nodes: {} });
  const currentNodeCount = getNodeCount(currentData);

  // Sécurité anti-reset : si l'arbre actuel contient des membres,
  // on refuse d'écrire un arbre vide sauf quand /reset le demande explicitement.
  if (nextNodeCount === 0 && currentNodeCount > 0 && !options.allowEmpty) {
    throw new Error("Sécurité anti-reset : refus d’écraser un arbre existant par un arbre vide.");
  }

  writeJson(DATA_FILE, data);

  if (nextNodeCount > 0) {
    mirrorLastGoodFamily(data);
  }
}
...
<@${avec.id}>, tu peux accepter ou refuser cette demande.`,
    ...
    if (interaction.commandName === "reset") {
      ...
      saveFamily({ nodes: {} }, { allowEmpty: true });
