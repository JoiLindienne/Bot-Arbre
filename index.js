...
if (!fs.existsSync(DATA_FILE)) fs.writeFileSync(DATA_FILE, '{"nodes":{}}');
if (!fs.existsSync(REQUESTS_FILE)) fs.writeFileSync(REQUESTS_FILE, '{"requests":{}}');
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("supprimer")
    .setDescription("Te supprimer de l’arbre")
    .addUserOption(o =>
      o.setName("qui")
        .setDescription("Ton @")
        .setRequired(true)
...
function getBlacklist() {
  const txt = fs.readFileSync(BLACKLIST_FILE, "utf8");
  const line = txt.split("\n").find(l => l.startsWith("PERSONNES_BLACKLIST=")) || "";
  return line
    .replace("PERSONNES_BLACKLIST=", "")
    .split(",")
    .map(x => x.trim())
    .filter(Boolean);
}
...
async function drawNode(ctx, guild, id, pos) {
  ...
  ctx.font = "600 18px Arial Rounded MT Bold, Arial";
...
async function drawTree(data, guild) {
  ...
  ctx.fillStyle = "#ffffff";
  ctx.font = "68px Arial Rounded MT Bold, Arial";
  ctx.textAlign = "center";
